...
#'
#' @examples
summarize_periods_backward <- function(criteria_results,
                              range_in_years,
                              ...) {
  ...
  df_ts <- expand.grid(site_summary_segment = segments, group_lower = groups, date = dates) %>%
  ...
  for(g in groups) {
    for(site in segments) {
      ...
      for(d in dates) {
        ts <- df_ts %>%
          dplyr::filter(group_lower == g & site_summary_segment == site) %>%
          dplyr::filter(between(date, d-365*range_in_years, d)) %>%
          ...
          dplyr::mutate(start_date = d-365*range_in_years,
                        end_date = d)
