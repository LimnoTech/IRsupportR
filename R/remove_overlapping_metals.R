...
remove_overlapping_metals <- function(df) {
  ...
  df_metals <- df %>%
    ...
    dplyr::mutate(form = tidyr::replace_na(form, "total")) %>%
    dplyr::group_by(station, date, group_lower) %>%
    dplyr::summarize(overlapping_metals = n(),
                     n_total = sum(form == "total"),
                     n_diss = sum(form == "dissolved")) %>%
    dplyr::mutate(n_total = tidyr::replace_na(n_total, 0)) %>%
    dplyr::mutate(n_diss = tidyr::replace_na(n_diss, 0)) %>%
    dplyr::right_join(df) %>%
    tidyr::separate(parameter, c("metal", "form"), remove=F) %>%
    dplyr::mutate(form = tidyr::replace_na(form, "total")) %>%
    dplyr::mutate(metals_flag = case_when(overlapping_metals > 1 & n_diss > 0 & form == "total" ~ "remove",
                                          TRUE ~ "-")) %>%
