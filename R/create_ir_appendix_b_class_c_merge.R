...
create_ir_appendix_b_class_c <- function (my_decision_logic, five_year_start_date) {
  ...
  df <- df %>%
    dplyr::mutate(number_of_samples_since_last_class_c_exceedance = dplyr::case_when(pollutant_name %in% pollutants_c_merge ~ "N/A – no Class C WQ criteria",
                                                                                     TRUE ~ number_of_samples_since_last_class_c_exceedance))

  df <- df %>%
    dplyr::mutate(cmc_criterion = dplyr::case_when(pollutant_name %in% pollutants_c_merge_cmc ~ "N/A – no Class C CMC WQ criteria",
                                                   TRUE ~ cmc_criterion))

  #Create new data frame for values that need to be merged
  mergeValues <- df %>%
    filter(number_of_samples_since_last_class_c_exceedance == "N/A – no Class C WQ criteria")

  mergeValues_cmc <- df %>%
    filter(cmc_criterion == "N/A – no Class C CMC WQ criteria")
  ...
  mergeValues_cmc <- mergeValues_cmc %>%
    dplyr::rename("Waterbody" = waterbody_segment,
                  "Pollutant" = pollutant_name,
                  "Pollutant Group" = pollutant_group,
                  "Test Fraction " = test_fraction,
  ...
  write.xlsx(mergeValues_cmc, file = "output/mergedrows_cmc.xlsx", rowNames = F)

  mv <- loadWorkbook(file = "output/mergedrows.xlsx")
  mv_cmc <- loadWorkbook(file = "output/mergedrows_cmc.xlsx")
  ...
  for(i in 1:nrow(mergeValues)) {
    ...
    if(x == "N/A – no Class C WQ criteria"){
      mergeCells(mv, 1, cols = 13:18, rows = i+1)
    }
  }

  for(i in 1:nrow(mergeValues_cmc)) {
    x <- mergeValues_cmc[i,16]
    if(x == "N/A – no Class C CMC WQ criteria"){
      mergeCells(mv_cmc, 1, cols = 16:17, rows = i+1)
    }
  }
